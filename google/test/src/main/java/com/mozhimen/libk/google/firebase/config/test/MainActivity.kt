...
import com.mozhimen.configk.google.ConfigKGoogleMgr
...
class MainActivity : BaseActivityVB<ActivityMainBinding>() {
    ...
    override fun initView(savedInstanceState: Bundle?) {
        ...
        ConfigKGoogleMgr.init()
        ConfigKGoogleMgr.setDefaultsAsync(R.xml.remote_config_defaults)
        ConfigKGoogleMgr.addOnConfigUpdateListener { configUpdate ->
            Log.d(TAG, "Updated keys: " + configUpdate.updatedKeys)
            if (configUpdate.updatedKeys.contains(WELCOME_MESSAGE_KEY)) {
                ConfigKGoogleMgr.activate().addOnCompleteListener {
    ...
    private fun fetchWelcome() {
        displayWelcomeMessage()
        ConfigKGoogleMgr.fetchAndActivate()
    ...
    private fun displayWelcomeMessage() {
        vb.welcomeTextView.isAllCaps = ConfigKGoogleMgr.firebaseConfig[WELCOME_MESSAGE_CAPS_KEY].asBoolean()
        vb.welcomeTextView.text = ConfigKGoogleMgr.firebaseConfig[WELCOME_MESSAGE_KEY].asString()
    }
}
